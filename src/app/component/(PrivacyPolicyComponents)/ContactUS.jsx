import React from "react";

const ContactUS = () => {
  return (
    <div style={styles.container} className="w-full p-4 md:p-8">
      <div style={styles.card} className="p-3 md:p-7 lg:p-9">
        <h2
          style={styles.heading}
          className="text-sm XS1:text-base md:text-lg lg:text-xl font-gotham"
        >
          Contact Us
        </h2>
        <p
          style={styles.paragraph}
          className="text-justify text-sm md:text-base lg:text-lg font-gotham font-light"
        >
          <h2
            style={styles.sub_heading}
            className="text-sm md:text-base lg:text-lg font-gotham font-light"
          >
            Online Booking
          </h2>
          <p className="text-base py-2">
            If you have any questions about this Privacy Policy, your personal
            data, or your rights, you can contact us at 247@cherryflight.com.
            You may also reach us by phone at +92 42 357 82 501-3 or write to us
            at 247@cherryflight.com.
          </p>
        </p>
      </div>
    </div>
  );
};

const styles = {
  container: {
    backgroundColor: "#f8f9fa",
    display: "flex",
    justifyContent: "center",
  },
  card: {
    backgroundColor: "#fff",
    maxWidth: "800px",
    boxShadow: "0 4px 12px rgba(0,0,0,0.1)",
    borderRadius: "4px",
    fontFamily: "Arial, sans-serif",
    fontSize: "16px",
    lineHeight: "1.6",
  },
  heading: {
    fontWeight: "bold",
    marginBottom: "1rem",
    borderBottom: "3px solid #f28323",
    display: "inline-block",
    paddingBottom: "4px",
    color: "rgb(30, 58, 138)",
  },
  sub_heading: {
    fontWeight: "400",
    color: "rgb(30, 58, 138)",
  },
  paragraph: {
    marginBottom: "1rem",
    justifyContent: "justify",
    textAlign: "justify",
  },
};

export default ContactUS;
